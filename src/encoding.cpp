...
namespace huffman {
    ...
    void binary_io::read_bits(std::ifstream& input, huffman_tree& tree, const std::string &filename) {
        ...
        while (input.get(buf)) {
            ...
            for (int j = 7; j >= 0; --j) {
                ...
                else if (bit == 1 && tree.get_root() -> get_left_child() == nullptr && tree.get_root() -> get_right_child() == nullptr) {
                    counter_chars += 1;
    ...
    size_t binary_io::get_compressed_file_size() const {
        return compressed_file_size_;
    }

    size_t binary_io::get_frequency_table_size() const {
        return frequency_table_size_;
    }

    size_t binary_io::get_not_compressed_file_size() const {
        return not_compressed_file_size_;
    }
